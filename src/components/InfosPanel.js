...
const InfosPanel = ({ hidePanel, panelInfos }) => {

    if (!panelInfos) {
        return null;
    }

    const { id, name, size, rotationSpeed, revolutionSpeed } = panelInfos;

    return (
        <div id="panel">
            <div id="panel-header">
                <div>{name}</div>
                <AiFillCloseCircle onClick={hidePanel} />
                {/* <svg
                    onClick={hidePanel}
                    width="24px"
                    height="24px"
            ...
            <div className="details">Place from Sun: <b>{
                id === 1 ? `${id}st`
                    : id === 2 ? `${id}nd`
                        : id === 3 ? `${id}rd`
                            : `${id}th`}</b> planet</div>

            <div className="details">Orbital period: <b>{rotationSpeed}</b> days</div>
            <div className="details">Revolution period: <b>{revolutionSpeed}</b> days</div>
            <div className="details">Axis size: <b>{size}</b> km</div>
